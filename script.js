...
var searchdetail =document.getElementById('animedetail')
...
//--- add to dbเพิ่มข้อมูลลงดาต้าเบส
...
//--- รายการที่ชอบ แสดงข้อมูลมีปุ่มdetail delete
...
function deleteani (id) {
    fetch(`https://se104-project-backend.du.r.appspot.com/movie?id=632110335&&movieId=${id}`,{
        ...
        { 
            ...
            { alert('ไม่มีการ์ตูนเรื่องนี้ในฐานข้อมูล') 
    })
}

//--- ฟังชันdetail 
function detail(id){
    fetch(`https://se104-project-backend.du.r.appspot.com/movie/632110335/${id}`)
    .then(response => { 
        if (response.status === 200)
        { 
            return response.json() 
